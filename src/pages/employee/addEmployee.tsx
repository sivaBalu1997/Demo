...
const AddEmployee = () => {
  ...
  const employee: EmployeeIdByDetails | null = useSelector(
    (state: RootState) => state.employee.employeeByIdDetails || "null"
  );

  const employeeByIdDetailsLoading = useSelector(
    (state: RootState) => state.employee.employeeByIdDetailsLoading
  );

  const refreshNewPin = useSelector(
    (state: RootState) => state.employee.refreshPin
  );
  const isGettingNewPin = useSelector(
    (state: RootState) => state.employee.isGettingNewPin
  );

  const roleFunctionFetching = useSelector(
    (state: RootState) => state.employee.roleFunctionFetching
  );
  const rolesAndFunctions: RolesAndFunctions[] = useSelector(
    (state: RootState) => state.employee.rolesAndFunctions
  );
  const [localRoleFunctionFetching, setLocalRoleFunctionFetching] =
    useState(false);
  ...
  useEffect(() => {
    ...
    employee,
    employeeByIdDetailsLoading,
    localRoleFunctionFetching,
    dataFetching,
  ]);
  ...
  const employeeAdded = useSelector(
    (state: RootState) => state.employee.employeeAdded
  );
  ...
  const employeeUpdateLoading = useSelector(
    (state: RootState) => state.employee.employeeUpdateLoading
  );
  const employeeUpdated = useSelector(
    (state: RootState) => state.employee.employeeUpdated
  );
  const employeeActionCompleted = useSelector(
    (state: RootState) => state.employee.employeeActionCompleted
  );
  const addEmployeeFailure = useSelector(
    (state: RootState) => state.employee.addEmployeeFailure
  );
  const updateEmployeeFailure = useSelector(
    (state: RootState) => state.employee.updateEmployeeFailure
  );
  ...
  const userBranchName = useSelector(
    (state: RootState) => state.auth?.restaurantDetails?.branchName || null
  );

  const [countryCode, setCountryCode] = useState("");
  const [isDropdownDisabled, setIsDropdownDisabled] = useState(false);
  const [nickname, setNickname] = useState(
    editEmployee ? employee?.toUseNickName : false
  );
  ...
  >([]);
  const [restaurantBranchDefaultValue, setRestaurantBranchDefaultValue] =
  ...
  const branchOptions = restaurantBranch
    ...
    : [];
  const employeeRoleAndFunctionsLoading = useSelector(
    (state: RootState) => state.employee.employeeRoleAndFunctionsLoading
  );
  ...
  useEffect(() => {
    ...
    if (restaurantDetails) {
      ...
      const isOutletHasDropDown =
        credentials?.role && credentials?.role === "Restaurant_Owner"
          ? true
          : credentials?.role === "Regional_Employee"
          ? true
          : credentials?.role === "Magil_Admin"
          ? true
          : false;
  ...
  const roles = useSelector(
    (state: RootState) => state?.employee?.employeeRoleAndFunctions
  );
  ...
  const handleNickNameCheckboxChange = (
  ...
  const getRole = () => {
    ...
    const decodedToken: any = credentials?.accessToken
  ...
  const initializeCheckedFunctions = (
    rolesAndFunctions: RolesAndFunctions[]
  ) => {
  ...
  const isModuleChecked = (moduleName: string) => {
    const module = (roles as RolesAndFunctions[]).find(
      (module) => module?.module.toLowerCase() === moduleName.toLowerCase()
    );
    if (!module) return false;

    return module.functionality?.every(
      (func) =>
        func?.displayName &&
        checkedFunctions.includes(func?.displayName?.toLowerCase())
    );
  };
  ...
  const handleModuleCheckboxChange = (moduleName: string) => {
    ...
    setCheckedFunctions((prevCheckedFunctions) => {
      if (
        moduleFunctions.every((func) => prevCheckedFunctions.includes(func))
      ) {
        return prevCheckedFunctions.filter(
          (func) => !moduleFunctions.includes(func)
        );
      } else {
        return [
          ...prevCheckedFunctions,
          ...moduleFunctions.filter(
            (func) => !prevCheckedFunctions.includes(func)
          ),
        ];
      }
    });
  ...
  const isDefaultActionsUpdated = (
    ...
    const selectedFunctionNames = selectedFunctions.map(
      (func) => func?.toLowerCase() ?? ""
    );
    const hasExtraFunctions = selectedFunctionNames.some(
      (func) => !defaultFunctions.includes(func)
    );
    const hasMissingFunctions =
      selectedFunctionNames.length !== defaultFunctions.length;
  ...
  const onSubmit = (formValues: any) => {
    ...
    const rolesAndFunctions = (roles as RolesAndFunctions[])
      .map((module) => {
        const moduleFunctions = module?.functionality
          .filter(
            (func) =>
    ...
    const transformedRolesAndFunctions: string[] = (
      rolesAndFunctions as PayloadRolesType[]
    ...
    formValues = {
      ...
      mobileNumber:
        formValues?.mobileNumber && formValues?.mobileNumber?.length > 0
          ? removeDashes(formValues.mobileNumber)
          : "",
      address:
        formValues.address1 || formValues.address2
          ? `${formValues.address1 || ""} ${formValues.address2 || ""}`.trim()
          : null,
      ...
      locationId: restaurantBranch.find((outlet) =>
  ...
  const splitAddress = (address: string) => {
    ...
    return [address.slice(0, splitIndex), address.slice(splitIndex + 1)];
  };
            ...
            <div className="menu-details-form">
              <div className="primary-sec">
                <div className="flexContainer">
                  <div>
                    <label
                      className={errors.firstName ? "errorLabel" : "inputLabel"}
                    >
                      First Name*
                    </label>
                    <Controller
                ...
                <div>
                  <label
                    className={errors.nickName ? "errorLabel" : "inputLabel"}
                  >
                    Nick Name
                  </label>
                  <Controller
                ...
                <div className="checkBox" style={{ marginTop: "-20px" }}>
                  <label>
                    ...
                    />
                    <p>
                      Utilize a nickname as needed in all forthcoming activities
                    </p>
                  </label>
                </div>
                ...
                <div className="flexBox">
                  <div>
                    <label
                      className={
                        errors.mobileNumber ? "errorLabel" : "inputLabel"
                      }
                    >
                      Phone
                    </label>
                    <div
                      className={
                        errors.mobileNumber
                          ? "num phoneErrorInput countryCodeMo"
                          : "phoneContainer countryCodeMo"
                      }
                    >
                      <div
                  ...
                  >
                    <label
                      className={errors.email ? "errorLabel" : "inputLabel"}
                    >
                      Email
                    </label>
                    <div>
                      <Controller
                        ...
                        render={({ onChange, value, name }) => (
                          <input
                            ...
                            onChange={onChange}
                            className={
                              errors.email ? "emailErrorInput" : "emailInput"
                            }
                ...
                <div className="flexContainer">
                  <div>
                    <label
                      className={errors.outlet ? "errorLabel" : "inputLabel"}
                    >
                      Outlet*
                    </label>
                    <div
                            ...
                            <CustomDropdown
                              ...
                              onSelect={(outletSelected: {
                                ...
                                if (restaurantBranch && outletSelected) {
                                  const selectedBranch = restaurantBranch.find(
                                    (branch) =>
                                      branch?.locationName?.includes(
                  ...
                  <div>
                    <label
                      className={errors.role ? "errorLabel" : "inputLabel"}
                    >
                      Roles*
                    </label>
                    <div
                    ...
                    >
                      ...
                      {selectedRole && (
                        <div className="roleFunction">
                          <p onClick={() => setOpenFuction(!openFunction)}>
                      ...
                      {openFunction && (
                        <div className="dropDownContainer">
                          <div className="functionsDropDown" ref={dropdownRef}>
                            <p
                              className="dropDownTitle fixedTitle"
                              style={{ textAlign: "center", fontWeight: 500 }}
                            >
                              Roles/Functions
                            </p>
                            {employeeRoleAndFunctionsLoading ? (
                              <p className="rfLoading">
                                Loading, Please wait!!
                              </p>
                            ) : (
                              <div className="checkList">
                                <div className="checkBoxContainer">
                                  {(roles as RolesAndFunctions[]).map(
                                    (module) => (
                                      <div
                            ...
                            <div className="functionBtn">
                              ...
                              <button
                                className="saveBtn"
                                value="Save"
                                onClick={() => setOpenFuction(!openFunction)}
                                style={{ border: "none" }}
                              >
                                Save
                              </button>
                      ...
                      {errors.role && (
                        <p className="employeeError">{errors.role?.message}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flexContainer">
                  <div>
                    <label
                      className={errors.userId ? "errorLabel" : "inputLabel"}
                    >
                      User Id*
                    </label>
                    <div>
                  ...
                  <div>
                    <label
                      className={errors.password ? "errorLabel" : "inputLabel"}
                    >
                      Password*
                    </label>
                    <div>
                ...
                >
                  <div
                    ...
                    style={
                      hasPinErrors
                        ? { fontSize: "15px", color: " #FF0505" }
                        : { fontSize: "15px", color: "#646262" }
                    }
                  >
                    Create Pin*
                    <img src={info} alt="info" className="infoToolTip" />
                    <span className="hoverImage">
                      The four digit pin will be used for validating the user
                    </span>
                  </div>
            ...
            <div className="btn form-cta">
              ...
              <span>
                <input
                  ...
                  type="submit"
                  value={
                    addEmployeeLoading || employeeUpdateLoading ? "" : "Save"
                  }
                  disabled={saveBtndisable}
                />
                {(addEmployeeLoading || employeeUpdateLoading) && (
