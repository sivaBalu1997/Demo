...
const CustomerInsights = () => {
  ...
  const [showLiveNetSaleTooltip, setShowLiveNetSaleTooltip] =
    ...
    <div style={{ display: "flex", flexDirection: "row", width: "100%" }}>
      ...
      >
        ...
        <div className="l-live-boxes-container">
          <div
            className={
              isExpanded ? "l-live-inner-box-expanded" : "l-live-inner-box"
            }
          >
            <div className="l-live-box">
              {liveDiscountDataAPIRedux && liveDiscountDataAPIRedux > 0 ? (
                <h2>
                  {countryCode === "US" ? "$" : "₹"}
                  {liveDiscountDataAPIRedux &&
                    liveDiscountDataAPIRedux > 0 &&
                    liveDiscountDataAPIRedux}
                </h2>
              ) : (
                // <p className='s-live-no-data'>No data found !</p>
            ...
            <div className="l-live-box">
              {liveRefundsAPIRedux && liveRefundsAPIRedux > 0 ? (
                <h2>
                  {countryCode === "US" ? "$" : "₹"}
                  {liveRefundsAPIRedux &&
                    liveRefundsAPIRedux > 0 &&
                    liveRefundsAPIRedux}
                </h2>
              ) : (
                // <p className='s-live-no-data'>No data found !</p>
            ...
            <div className="l-live-box">
              {liveOpenSalesDataAPIRedux && liveOpenSalesDataAPIRedux > 0 ? (
                <h2>
                  {countryCode === "US" ? "$" : "₹"}
                  {liveOpenSalesDataAPIRedux &&
                    liveOpenSalesDataAPIRedux > 0 &&
                    liveOpenSalesDataAPIRedux}
                </h2>
              ) : (
                // <p className='s-live-no-data'>No data found !</p>
            ...
            <div className="l-live-box">
              {liveNetSalesAPIRedux && liveNetSalesAPIRedux > 0 ? (
                <h2>
                  {countryCode === "US" ? "$" : "₹"}
                  {liveNetSalesAPIRedux &&
                    liveNetSalesAPIRedux > 0 &&
                    liveNetSalesAPIRedux}
                </h2>
              ) : (
                // <p className='s-live-no-data'>No data found !</p>
        ...
        <div className="live-orders-non-dine-in">
          <Table
            ...
            tableData={
              liveOrderNonDineInAPIRedux &&
              liveOrderNonDineInAPIRedux?.length > 0 &&
              liveOrderNonDineInAPIRedux
            }
            viewType="full"
            recordsPerPage={RECORDS_PER_PAGE_LIMIT}
